    ...
    <Layout>
      ...
      <section className="inner-hero-sec">
        ...
        {heroImage && (
          <GatsbyImage
            image={heroImage}
            alt={heroDesktop?.altText || "Blog Hero Image"}
      ...
      {/* ================= BLOG LISTING ================= */}
       ...
       <section className="latest-update pseudo-animate blog-page-main">
      <div className="container">
       
          {posts.map(({ node }) => {
            // Fetch desktop and mobile images for blog banners
            const desktopImage = node.blogBanner?.blogImageDesk?.node?.gatsbyImage
            const mobileImage = node.blogBanner?.blogImageMob?.node?.gatsbyImage

            return (
              <div className="blog-page " key={node.id}>
                <article className="blog-card">

                  <Link to={node.uri} className="blog-link">
                    <div className="blog-image">
                      {/* Desktop Image */}
                      {desktopImage && (
                        <GatsbyImage
                          image={desktopImage}
                          alt={node.blogBanner?.blogImageDesk?.node?.altText || node.title}
                      ...
                      {mobileImage && (
                        <GatsbyImage
                          image={mobileImage}
                          alt={node.blogBanner?.blogImageMob?.node?.altText || node.title}
                          className="blog-hero-image"
                        />
                      )}
                    </div>
                        ...
                        <div className="social">
                          <ul>
                            <li className="icon insta">
                              <a href="https://www.instagram.com/drgulshanrohra/">
